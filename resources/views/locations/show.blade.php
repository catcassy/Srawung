...
<div class="relative h-52 bg-purple-50 overflow-hidden">
  @if($location->foto)
  <img src="{{ asset('storage/'.$location->foto) }}" alt="{{ $location->nama }}" class="w-full h-full object-cover">
  @else
  <div class="w-full h-full flex items-center justify-center text-6xl">{{ $location->kategoriIcon() }}</div>
  @endif
  <span class="absolute bottom-3 left-4 bg-white/90 text-xs font-bold px-3 py-1.5 rounded-xl text-gray-700">{{ $location->kategoriIcon() }} {{ ucfirst($location->kategori) }}</span>
</div>
<div class="px-4 py-5">
  <h2 class="text-xl font-black text-gray-900 mb-1">{{ $location->nama }}</h2>
  ...
  @if($location->deskripsi)<p class="text-sm text-gray-700 leading-relaxed mb-4">{{ $location->deskripsi }}</p>@endif
...
@endif
  <div class="pt-4 border-t border-purple-50 flex items-center gap-2.5">
    <img src="{{ $location->user->avatarSrc() }}" class="w-9 h-9 rounded-xl object-cover" alt="">
    <div>
      <span class="text-xs text-gray-400">Direkomendasikan oleh</span>
      <a href="{{ route('profil',$location->user) }}" class="block text-sm font-bold text-gray-900 hover:text-purple-700">{{ $location->user->name }}</a>
    </div>
    <span class="ml-auto text-xs text-gray-400">{{ $location->created_at->diffForHumans() }}</span>
  </div>
</div>
