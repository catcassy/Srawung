...
<title>@yield('title','Srawung')</title>
...
<div class="max-w-screen-xl mx-auto flex min-h-screen">
  ...
  <aside class="hidden md:flex flex-col w-16 lg:w-64 flex-shrink-0 sticky top-0 h-screen px-2 lg:px-4 py-5 border-r border-purple-100 bg-white overflow-y-auto no-scroll">
    ...
    {{-- Logo --}}
   <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-2 lg:px-3 mb-6">
...
src="{{ asset('images/srawung-logo.png') }}"
    ...
    <nav class="flex flex-col gap-1 flex-1">
      ...
      <a href="{{ route('lokal') }}" class="nav-link {{ request()->routeIs('lokal') ? 'active' : '' }}">
        <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M17.657 16.657L13.414 20.9a2 2 0 01-2.828 0l-4.243-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
        <span class="hidden lg:block">Thread Lokal</span>
      </a>
      <a href="{{ route('ketemu.index') }}" class="nav-link {{ request()->routeIs('ketemu.*') ? 'active' : '' }}">
        ...
        <span class="hidden lg:flex ml-auto w-5 h-5 rounded-full bg-orange-400 text-white text-xs items-center justify-center font-bold">{{ auth()->user()->pendingMeetups() }}</span>
        @endif
      </a>
      ...
      <a href="{{ route('pesan.index') }}" class="nav-link {{ request()->routeIs('pesan.*') ? 'active' : '' }}">
        ...
        <span class="hidden lg:flex ml-auto w-5 h-5 rounded-full bg-red-500 text-white text-xs items-center justify-center font-bold">{{ auth()->user()->unreadMessages() }}</span>
        @endif
      </a>
      <a href="{{ route('profil', auth()->user()) }}" class="nav-link {{ request()->routeIs('profil') && request()->route('user') && request()->route('user')->id === auth()->id() ? 'active' : '' }}">
      ...
      <a href="{{ route('post.create') }}" class="btn-primary mt-3 justify-center lg:justify-start">
    ...
    <div class="mt-auto pt-3 border-t border-purple-100">
      <a href="{{ route('profil', auth()->user()) }}" class="flex items-center gap-2.5 p-2 rounded-xl hover:bg-purple-50 transition">
        <img src="{{ auth()->user()->avatarSrc() }}" class="w-9 h-9 rounded-xl object-cover flex-shrink-0 shadow-sm" alt="">
        ...
        <span class="hidden lg:block text-xs px-2 py-0.5 rounded-full {{ auth()->user()->mode==='anonim' ? 'bg-slate-100 text-slate-500' : 'bg-purple-100 text-purple-600' }}">
          {{ auth()->user()->mode }}
        </span>
      </a>
      <div class="hidden lg:flex justify-between px-2 pt-1">
        <a href="{{ route('profile.edit') }}" class="text-xs text-gray-400 hover:text-purple-600 transition">Pengaturan</a>
  ...
  <main class="flex-1 min-w-0 border-x border-purple-100 bg-white pb-16 md:pb-0">
    ...
    @endif
    @if(session('info'))
    <div class="bg-blue-50 border-b border-blue-100 text-blue-700 text-sm px-4 py-2.5">ℹ️ {{ session('info') }}</div>
    @endif
    @if($errors->any())
  ...
  <aside class="hidden xl:flex flex-col w-80 flex-shrink-0 px-4 py-5 sticky top-0 h-screen overflow-y-auto no-scroll gap-4">
    ...
    <div class="card overflow-hidden">
      ...
      <div class="p-3">
        <a href="{{ route('ketemu.index') }}" class="block w-full text-center bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold py-2 rounded-lg transition">
    ...
    <div class="card p-4">
      ...
      @foreach(\App\Models\Location::with('user')->latest()->take(4)->get() as $loc)
      <a href="{{ route('lokasi.show', $loc) }}" class="flex items-center gap-3 py-2.5 border-b border-purple-50 last:border-0 hover:bg-purple-50 -mx-2 px-2 rounded-xl transition">
        <div class="w-10 h-10 rounded-xl overflow-hidden bg-purple-50 flex-shrink-0 flex items-center justify-center">
          @if($loc->foto)
          <img src="{{ asset('storage/'.$loc->foto) }}" class="w-full h-full object-cover" alt="">
          @else
          <span class="text-lg">{{ $loc->kategoriIcon() }}</span>
          @endif
        </div>
        <div class="min-w-0 flex-1">
          <div class="text-xs font-bold text-gray-900 truncate">{{ $loc->nama }}</div>
          <div class="text-xs text-gray-400 truncate">{{ $loc->alamat }}</div>
        </div>
      </a>
      @endforeach
      <a href="{{ route('lokasi.index') }}" class="block text-xs text-purple-600 hover:underline mt-2 font-medium">Lihat semua →</a>
    </div>

    {{-- Saran ikuti --}}
    <div class="card p-4">
      <h3 class="font-bold text-gray-900 text-sm mb-3">Mungkin kamu kenal</h3>
      @foreach(\App\Models\User::where('id','!=',auth()->id())->inRandomOrder()->take(4)->get() as $su)
      <div class="flex items-center gap-2.5 py-2.5 border-b border-purple-50 last:border-0">
        <a href="{{ route('profil', $su) }}" class="flex-shrink-0">
          <img src="{{ $su->avatarSrc() }}" class="w-9 h-9 rounded-xl object-cover hover:opacity-80 transition" alt="{{ $su->name }}">
        </a>
        <div class="min-w-0 flex-1">
          <a href="{{ route('profil', $su) }}" class="block text-xs font-bold text-gray-900 hover:text-purple-700 transition truncate">{{ $su->name }}</a>
          <div class="text-xs text-gray-400">{{ $su->username }}</div>
        </div>
        @if(!auth()->user()->isFollowing($su))
        <form action="{{ route('follow.toggle', $su) }}" method="POST">@csrf
  ...
  <div class="relative" id="mobile-profile-wrap">
    ...
    <div id="mobile-menu" class="hidden absolute top-12 left-0 w-64 bg-white rounded-2xl shadow-xl border border-purple-100 z-50 overflow-hidden">
      <div class="px-4 py-3 border-b border-purple-50">
        ...
        <div class="text-xs text-gray-400">{{ auth()->user()->username }}</div>
        <span class="text-xs px-2 py-0.5 rounded-full mt-1 inline-block {{ auth()->user()->mode==='anonim' ? 'bg-slate-100 text-slate-500' : 'bg-purple-100 text-purple-600' }}">{{ auth()->user()->mode }}</span>
      </div>
      <nav class="py-1">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-purple-50">
        ...
        <a href="{{ route('profil', auth()->user()) }}" class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-purple-50">
        ...
        <a href="{{ route('ketemu.index') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-purple-50">
          <svg class="w-4 h-4 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg> Srawung Ketemu
        </a>
        <a href="{{ route('lokasi.index') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-purple-50">
        ...
        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-purple-50">
        ...
        <div class="border-t border-gray-100 mt-1">
          <form action="{{ route('logout') }}" method="POST" class="px-4 py-3">@csrf
